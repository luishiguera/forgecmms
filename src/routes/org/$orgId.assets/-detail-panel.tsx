...
import { locationQueryOptions } from "@/lib/queries/locations";
import {
	tagsQueryOptions,
...
import { AssetForm, buildPartialUpdateRequest } from "./-asset-form";
import type { AssetFormValues } from "./-types";

type AssetTab = "overview" | "related" | "attachments";
...
export function AssetDetailPanel({
	...
	} = useQuery(assetQueryOptions(orgId, assetId));

	const locationId = asset?.location_id;
	const { data: assetLocation } = useQuery({
		...locationQueryOptions(orgId, locationId ?? 0),
		enabled: !!locationId,
	});

	const parentAssetId = asset?.parent_asset_id;
	const { data: parentAsset } = useQuery({
		...assetQueryOptions(orgId, parentAssetId ?? 0),
	...
	const updateMutation = useUpdateAssetMutation(orgId);
	...
	const handleAutoSave = async (values: AssetFormValues) => {
	...
	const currentTagIds = asset?.tags.map((t) => t.id) ?? [];
	...
	return (
		<div className="flex flex-1 flex-col min-w-0 overflow-hidden">
			<div className="shrink-0 border-b bg-card">
				...
				<Tabs
					value={activeTab}
					onValueChange={(val) => onTabChange(val as AssetTab)}
				>
					<TabsList variant="line" className="px-6">
						<TabsTrigger value="overview">
							{m.assets_tab_overview()}
						</TabsTrigger>
						<TabsTrigger value="related">{m.assets_tab_related()}</TabsTrigger>
						<TabsTrigger value="attachments">
							{m.assets_tab_attachments()}
						</TabsTrigger>
					</TabsList>
				</Tabs>
			</div>
