...
- Escape hatch in this repo: `useMountEffect` in `src/hooks/use-mount-effect.ts`
- Enforcement: convention and review. Biome does NOT ban `useEffect` in `biome.json`, so the linter will not catch a violation for you
...
It lives in `src/hooks/use-mount-effect.ts` and wraps `useEffect` with an empty
dependency array to make the intent explicit:
...
This suppression comment is the one comment the repo allows here. The project
rule in `CLAUDE.md` bans explanatory comments, not lint directives.
